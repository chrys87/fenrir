    ...
    def updateWatchdog(self, active, eventQueue, screenQueue):
        try:
            ...
            while active.value == 1:
                self.env['runtime']['processManager'].addCustomEventThread(self.encoderWatchdog, [screenQueue, currScreen], multiprocess = True, runOnce = True)                    
        ...
        screenQueue.put(-1)                                       
    def encoderWatchdog(self, active, eventQueue, paramList):
        currScreen = paramList[0]        
        currScreen = paramList[1]
        self.updateCharMap(currScreen) 
        lastEvent = None
        while active.value == 1:            
